...
class ResidualAttentionBlock_Coarse_Multi(nn.Module):
    def __init__(self, d_model: int, n_head: int, layer: int, attn_mask=None):
        super().__init__()
        self.layer = layer
...
class VisualTransformer_Coarse_multi(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.transformer = Transformer_Coarse_multi(width, layers, heads)        
        self.ln_post = LayerNorm(width)
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))
...
class VisualTransformer_multi(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.transformer = Transformer_multi(width, layers, heads)        
        self.ln_post = LayerNorm(width)
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.token_embedding_1 = nn.Embedding(vocab_size, transformer_width)
        self.token_embedding_2 = nn.Embedding(vocab_size, transformer_width)
        self.positional_embedding = nn.Parameter(torch.empty(self.context_length, transformer_width))
        self.positional_embedding_ = nn.Parameter(torch.empty(self.context_length, transformer_width))
        self.positional_embedding_1 = nn.Parameter(torch.empty(self.context_length, transformer_width))
        self.positional_embedding_2 = nn.Parameter(torch.empty(self.context_length, transformer_width))
        self.ln_final = LayerNorm(transformer_width)
        self.ln_final_ = LayerNorm(transformer_width)
        self.ln_final_1 = LayerNorm(transformer_width)
        self.ln_final_2 = LayerNorm(transformer_width)

        self.text_projection = nn.Parameter(torch.empty(transformer_width, embed_dim))
        self.text_projection_ = nn.Parameter(torch.empty(transformer_width, embed_dim))
        self.text_projection_1 = nn.Parameter(torch.empty(transformer_width, embed_dim))
        self.text_projection_2 = nn.Parameter(torch.empty(transformer_width, embed_dim))
    ...
    def initialize_parameters(self):
        ...
        if self.text_projection_1 is not None:
            nn.init.normal_(self.text_projection_1, std=self.transformer_Coarse_multi.width ** -0.5)
        if self.text_projection_2 is not None:
            nn.init.normal_(self.text_projection_2, std=self.transformer_multi.width ** -0.5)
    ...
    def encode_image(self, image, return_hidden=False, video_frame=-1):
        ...
        x = hidden[:, 0, :]
        x_Coarse = hidden_Coarse[:, 0,:]
        x_multi = hidden_multi[:, 0, :]
        x_coarse_multi = hidden_Coarse_multi[:, 0, :]
        
        if return_hidden:
            return x, hidden, x_Coarse, hidden_Coarse,x_multi,hidden_multi,x_coarse_multi,hidden_Coarse_multi

        return x, x_Coarse,x_multi,x_coarse_multi
    ...
    def encode_text(self, text, return_hidden=False):
        ...
        x_multi = self.token_embedding_1(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        x_coarse_multi = self.token_embedding_2(text).type(torch.float16)  # [batch_size, n_ctx, d_model]

        pos_emd_1 = self.positional_embedding_1[:x_multi.size(1), :].type(self.dtype)
        pos_emd_2 = self.positional_embedding_2[:x_coarse_multi.size(1), :].type(self.dtype)
        pos_emd_ = self.positional_embedding_[:x_coarse.size(1), :].type(self.dtype)
        pos_emd = self.positional_embedding[:x.size(1), :].type(self.dtype)

        x_multi = x_multi + pos_emd_1
        x_coarse_multi = x_coarse_multi + pos_emd_2
        ...
        x_multi = x_multi.permute(1, 0, 2)  # NLD -> LND
        x_multi = self.transformer_multi(x_multi)
        x_multi = x_multi.permute(1, 0, 2)  # LND -> NLD

        x_coarse_multi = x_coarse_multi.permute(1, 0, 2)  # NLD -> LND
        x_coarse_multi = self.transformer_Coarse_multi(x_coarse_multi)
        x_coarse_multi = x_coarse_multi.permute(1, 0, 2)  # LND -> NLD


        hidden = self.ln_final(x).type(self.dtype) @ self.text_projection
        hidden_coarse = self.ln_final_(x_coarse).type(self.dtype) @ self.text_projection_

        hidden_multi = self.ln_final_1(x_multi).type(self.dtype) @ self.text_projection_1
        hidden_coarse_multi = self.ln_final_2(x_coarse_multi).type(self.dtype) @ self.text_projection_2
        ...
        x_multi = hidden_multi[torch.arange(hidden_multi.shape[0]), text.argmax(dim=-1)]
        x_coarse_multi = hidden_coarse_multi[torch.arange(hidden_coarse_multi.shape[0]), text.argmax(dim=-1)]
    ...
    def _convert_weights_to_fp16(l):
        ...
        for name in ["text_projection_", "proj"]:
            if hasattr(l, name):
                ...
                if attr is not None:
                    attr.data = attr.data.half()
        for name in ["text_projection_1", "proj"]:
            if hasattr(l, name):
                attr = getattr(l, name)
                if attr is not None:
                    attr.data = attr.data.half()
        for name in ["text_projection_2", "proj"]:
            if hasattr(l, name):
                attr = getattr(l, name)
                if attr is not None:
                    attr.data = attr.data.half()
